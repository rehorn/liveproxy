...
function routeResponder(router, req, res, options) {
    options = options || {};
    var proxyHost = '';
    var route = route || getRouter(req.url);
    if (router) {
        // directly proxy
        if (router.action == '-') {
            console.log('[proxy]: ' + req.url.gray);
            goProxy(req, res, null, options);
        } else {
            // proxy to specified server
            console.log('[route]: '.yellow + req.url.grey);
            console.log('   -> ' + router.action);
            goProxy(req, res, router.action, options);
        }
    } else {
        // directly proxy
        console.log('[proxy]: ' + req.url.grey);
        goProxy(req, res, null, options);
    }
};
